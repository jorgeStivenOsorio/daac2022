...
class AdminUpdate extends StatefulWidget {
  static String id = "AdminUpdate";

  @override
  State<AdminUpdate> createState() => AdminUpdateState();
}

class AdminUpdateState extends State<AdminUpdate> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(child: formAdmin(admin!)),
    );
  }

  //Formulario de cliente
  Widget formAdmin(Administrador admin) {
    usuario = forms.textController(admin.usuario.toString());
    contrasena = forms.textController(admin.contrasena.toString());
    nombre = forms.textController(admin.nombre.toString());
    apellido = forms.textController(admin.apellido.toString());
    edad = forms.textController(admin.edad.toString());
    genero = admin.genero;
    return Form(
      key: _formKey,
      child: Column(
  ...
  updateAdmin(Administrador? admin) async {
    try {
      await firebase.collection('Usuario').doc(admin!.id.toString()).set({
  ...
  Widget buttonSubmit(context, Administrador? admin) {
    return Padding(
